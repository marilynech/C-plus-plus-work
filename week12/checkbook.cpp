...
int main(){
    ...
    for(int i = 0; i < totalChecks; i++){
    double value = (checkBook[i].getAmount());
    if(checkBook[i].gotCashed()){
      oldBalance = oldBalance - value;
      cashedCheck += value;
      cashedChecks.push_back(checkBook[i].getAmount());
    }else{
      if(checkBook[i].getAmount() != ""){
        notCashed.push_back(checkBook[i].getAmount());
      }
    }
    totalCash += value;
